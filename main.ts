...
const handleClick = (taskCheckbox) => {
    ...
    for (const task of tasks){
        ...
        if (task.firstChild?.isSameNode(taskCheckbox)){
            const completTarefa: HTMLInputElement = document.querySelector("#completas")! 
...
const saveTasks = () => {
    ...
    const completedTasks = document.getElementById('completas')?.innerHTML;
    const incompleteTasks = document.getElementById('incompletas')?.innerHTML;
